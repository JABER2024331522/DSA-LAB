...
int main()
{
    ...
    int abd[3000], b2[3000];
    double c2[3000], ans = 0.00, ans1 = 0.00, ans2 = 0.00;
    int i, n = 0, d2 = 0, cos, serial[3000];
    for (i = 0; i < 3000; i++)
        serial[i] = 0;
    int f1 = 0;
...
mainmenu:
        ...
        adminpanelchoice:
            ...
            adminchoise:
                ...
                if (adminchoise == 9)
                {
                    ...
                    for (i = 0; i < n; i++)
                    {
                        ...
                        while (temp != NULL)
                        {
                            if (temp->data == abd[i])
                            {
                                printf("|   %s      |   %d         |   %0.2f     |   %0.2f              |   %0.2f      \n", temp->foodname, b2[i], c2[i], c2[i] * 0.2, c2[i] * 0.1);
                                ans += c2[i];
                                ans1 += c2[i] * 0.2;
                                ans2 += c2[i] * 0.1;
                                pre(4);
                ...
                else if (adminchoise == 8)
                {
                    ...
                    br(5);
                    pre(4);
                    echo("All Employees Information\n\n");
                    pre(4);
                    printf(" Manager \n\n\n");
                    Sleep(250);
                    pre(4);
    ...
    fcquantity:
        ...
        {
            serial[get_food_choice] = serial[get_food_choice] + fcquantity;
            temp->quantity -= fcquantity;
        }
        b2[n] = fcquantity;
        if (mn == 0.00)
        {
        ...
        else
        {
            middle1();
            pre(4);
            printf("Choice food %s its price is %0.2f \n\n                                With previous items and this item total price would be %0.2f \n\n", temp->foodname, temp->price * fcquantity, mn + temp->price * fcquantity);
            pre(4);
            c2[n] = temp->price * fcquantity;
            n++;
            d2++;
